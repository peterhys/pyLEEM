...
class SEESAnalyzer(ProfileAnalyzer):
    ...
    def __init__(self, path, roi, sigma=10, scale=1):
        super().__init__(path, roi, sigma=sigma, scale=scale)

    def preprocess(self):
        """Preprocess SEES profile by filtering and finding onset."""
    ...
    def postprocees(self):
        """Postprocess SEES profile by calculating surface potential."""
        if self.is_calibrated:
            self.surface_potential = self.roi.peak_shift - (
                self.metadata["Start Voltage"][0]
                + self.onset_pos / self.roi.pixel_per_ev
            )
            self.onset = 0
        else:
            # Assuming peak shift is 0 and pixel per eV is 165
            self.surface_potential = -(
                self.metadata["Start Voltage"][0] + self.onset_pos / 165
            )
            self.onset = self.onset_pos

    def transform_abscissa(self):
        """Transform to electron energy in eV."""
        KE = (self.pixel - self.onset_pos) / self.roi.pixel_per_ev
        return KE, "Energy [eV]"

    def plot(self, ax, show_fit=False):
        ...
        if show_fit:
            ax.plot(
                [self.onset, self.abscissa[self.pk_idx]],
...
class SEESGroup(AnalyzerGroup):
    ...
    def __init__(self, paths, roi, sigma=10, scale=1):

        super().__init__(
            paths, analyzer=SEESAnalyzer, roi=roi, sigma=sigma, scale=scale
        )
    ...
    def calibrate(self, cal_params, plot=False):
        ...
        onset_pixels = self.get_attrs("onset_pos")
        start_voltages = self.get_metas("Start Voltage")

        if cal_params.get("pixel_per_ev", None) is None:
            pixel_per_ev = np.mean(np.diff(onset_pixels) / -np.diff(start_voltages))
        else:
            pixel_per_ev = cal_params["pixel_per_ev"]

        if cal_params.get("peak_shift", None) is None:
            peak_shift = np.mean(start_voltages + onset_pixels / pixel_per_ev)
        else:
            peak_shift = cal_params["peak_shift"]

        # Apply calibration to the ROI
        self.roi.calibrate(pixel_per_ev, peak_shift)
